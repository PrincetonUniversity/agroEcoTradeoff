...
afr <- raster("external/ext_data/africa_grid.tif")

# GS
gs <- raster("external/ext_data/gs.tif")
r <- raster(extent(afr))
res(r) <- res(gs)
...
writeRaster(afr10, filename = "external/ext_data/africa-grid-10km.tif", 
            datatype = "INT4S", overwrite = TRUE)  # 10 km grid #s
afr10 <- raster("external/ext_data/africa-grid-10km.tif")
afrmask10 <- !is.na(afr10)
afrmask10[which(values(afrmask10) == 0)] <- NA
writeRaster(afrmask10, filename = "external/ext_data/africa-mask-10km.tif", 
            datatype = "INT4S", overwrite = TRUE)
afrmask10 <- raster("external/ext_data/africa-mask-10km.tif") # 10 km Afr mask
...
rasterize(gscnt, afr10, field = "id", 
          filename = "external/ext_data/gs-countries.tif")
gs_cnt_r <- raster("external/ext_data/africa-grid-10km.tif")
...
lapply(cnames, function(x) {
  ...
  print(x)
  cntr <- crop(afr, cnt)
  cntr2 <- mask(cntr, mask = cnt, filename = paste0("external/ext_data/", x, "-grid.tif"), 
               datatype = "INT4S", overwrite = TRUE) 
})
names(cntr_l) <- cnames
cntr_l <- lapply(cnames, function(x) {
  fnm <- full_path("external/ext_data/", paste0(x, "-grid.tif"))
  brick(fnm)
})
...
projectRaster(from = cropylds_af, to = afr10, progress = "text", 
              filename = "external/ext_data/crop-yields.tif")
projectRaster(from = cropareas_af, to = afr10, progress = "text", 
              filename = "external/ext_data/crop-areas.tif")
projectRaster(from = potylds_af, to = afr10, progress = "text", 
              filename = "external/ext_data/potential-yields.tif")
cropylds_af_p <- brick("external/ext_data/crop-yields.tif")
cropsareas_af_p <- brick("external/ext_data/crop-areas.tif")
potylds_af_p <- brick("external/ext_data/potential-yields.tif")
```
...
projectRaster(unfarmed, to = afr10, filename = "external/ext_data/unfarmed.tif", 
              overwrite = TRUE)
# unfarmed <- raster("external/ext_data/africa-unfarmed.tif")
```
...
brick(stack(curprod), filename = "external/ext_data/current-production.tif", 
      overwrite = TRUE)
curprod <- brick("external/ext_data/current-production.tif")
...
brick(stack(potprod_curarea), 
      filename = "external/ext_data/potential-production-current-area.tif")
potprod_curarea <- brick("external/ext_data/potential-production-current-area.tif")
 
# unfarmed_ha <-  unfarmed * afha
# potprod <- sapply(1:nlayers(potylds_af_p), function(x) {
#   prod <- unfarmed_ha * potylds_af_p[[x]]
# })
# brick(stack(potprod), filename = "external/ext_data/potential-production.tif")
# potprod <- brick("external/ext_data/potential-production.tif")
...
brick(stack(carbon), filename = "external/ext_data/carbon.tif", overwrite = TRUE)
carbon <- brick("external/ext_data/carbon.tif")
...
writeRaster(af_mamdiv, filename = "external/ext_data/af_mammal_div.tif")
af_mamdiv <- raster("external/ext_data/af_mammal_div.tif")
writeRaster(gs_mamdiv, filename = "external/ext_data/gs_mammal_div.tif")
gs_mamdiv <- raster("external/ext_data/gs_mammal_div.tif")
```
...
gdalwarp(srcfile = af_glc@file@name, 
         dstfile = "external/ext_data/glc-africa-alb.tif", 
         t_srs = af@proj4string@projargs, te = bbox(afr)[1:4], 
         tr = c(1000, 1000), r = "near")
af_lc <- raster("external/ext_data/glc-africa-alb.tif")
...
af_nat_areas <- focal(af_nat_cov, w = w, fun = mean, na.rm = TRUE, 
                      filename = "external/ext_data/af-natural-cover.gri", 
                      progress = "text")
writeRaster(af_nat_areas, filename = "external/ext_data/af-natural-cover.tif")
#file.remove(dir("external/ext_data/", pattern = "af-natural-cover.gr"))
af_nat_areas <- raster("external/ext_data/af-natural-cover.tif")  # 1 km version
af_nat_areas10 <- aggregate(af_nat_areas, fact = 9)
resample(af_nat_areas10, y = afr10, 
         filename = "external/ext_data/af-natural-cover-10km.tif")
...
writeRaster(natbd, filename = "external/ext_data/af-cons-priorities-10km.tif", 
            overwrite = TRUE)
cons_p <- raster("external/ext_data/af-cons-priorities-10km.tif")
