...
yblist <- list(yb1 <- c(1, 1), 
               ...
               yb3 <- c(1.2, 1))
# prod_targ space
tnames <- c("maize", "cassava", "ground", "cotton", "soy", "pulse", "sunflower", "sugarcane", "wheat")
...
save(batch_test, file = paste0("external/output/batch/dt/", dnm, "/out_tables.rda"))
...
library(tradeoffMod)
setwd(full_path(proj_root("tradeoff_mod"), "tradeoff_mod"))
dnm <- dir("external/output/batch/dt")
...
```{r, echo = FALSE, message = FALSE, fig.align='center'}
closs <- nm_up((il$carbon$veg + il$carbon$soil * 0.25) / il$p_yield[[c(1, 5)]], c("maize", "soy"))
par(mar = c(0, 0, 0, 0), oma = c(1, 1, 1, 1))
...
fracr <- nm_up(brick("external/ext_data/ZA-crop-convert-fractions-base.tif"), il$cropnames)
