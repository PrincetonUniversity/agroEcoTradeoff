...
#' @export
convert_dt3 <- function(conv_prob, target, crop_frac, pot_yield, cropnames, base, 
                       ha, keep_index = FALSE) {
  ...
  T <- rep(-1, nrow(conv_dt)) #vector representing allocation of pixels to crops
  alloc_prob <- rep(0, nrow(conv_dt)) #vector of probabilities for each pixel in its current allocation
  for(i in 1:ncol(conv_prob)) {
    targ <- target[i, "target_newland"]
    conv_prob[[i]] <- conv_prob[[i]] - alloc_prob #subtract the probabilities of pixels already allocated elsewhere
    inds <- order(conv_prob[, i, with = FALSE], decreasing = TRUE) #sort the probabilities in descending order
    for (j in 0:sum(ifelse(cumsum(prod_dt[inds, i, with = FALSE]) < targ, 1, 0))) #select enough pixels to meet targ
      T[inds[j]] <- i
    alloc_prob[T == i] = conv_prob[[i]][T == i] #update the vector of probabilities for allocated pixels
  }
  
  for (i in 1:length(cropnames)) {
    conv_dt[, cropnames[i]] = 0
    conv_dt[T == i, cropnames[i]] = 1
  }
